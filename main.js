...
// ---- Реестр движков обработки ----
// Раньше запуск был жёстко зашит на один AUTOMAX KG для вообще всех моделей
// машин — ни в car_models, ни здесь никакой привязки "модель -> чем её
// обрабатывать" не было. Теперь каждая модель в car_models несёт поле
// engine (см. schema.sql), а этот реестр — единственное место, которое
// знает, как конкретный движок проверяется на наличие и запускается.
// Добавить новый движок в будущем — дописать сюда ещё один объект с теми же
// тремя методами, никакой другой код (renderer, IPC-обработчик ниже) трогать
// не придётся. isPresent/spawn специально методы объекта (не отдельные
// функции) — у будущих движков путь/способ запуска может быть устроен
// совсем иначе (не .bat через cmd.exe, а прямой .exe, например), и у каждого
// движка это инкапсулировано в нём самом.
const ENGINES = {
  automaxkg: {
    label: 'AUTOMAX KG',
    isPresent() {
      return isAutomaxKgPresent();
    },
    spawn(cols, rows) {
      // Та же осторожная логика кавычек/cwd, что была всегда для AUTOMAX KG
      // (см. комментарий у automaxkg-terminal-start ниже) — поведение для
      // существующих моделей не меняется ни на байт.
      return pty.spawn('cmd.exe', `/d /s /c ""${AUTOMAXKG_BAT_PATH}""`, {
        name: 'xterm-256color',
        cols: cols > 0 ? cols : 80,
        rows: rows > 0 ? rows : 30,
        cwd: AUTOMAXKG_DIR,
  ...
  // Следующий движок (например, для CHANGAN X5 Plus) добавляется сюда же —
  // архитектура на это и рассчитана. Пока не подключён: нет ни программы,
  // ни данных о том, как именно её запускать.
};

// Неизвестный/пустой engineId (например, старая версия renderer без этого
// поля, или модель из car_models без движка по какой-то причине) — тихо
// откатываемся на automaxkg, а не падаем. Это тот же принцип "не ломать
// существующее поведение", что и default 'automaxkg' в схеме БД.
function getEngine(engineId) {
  return ENGINES[engineId] || ENGINES.automaxkg;
}
...
// Запускает движок обработки (по умолчанию AUTOMAX KG, см. реестр ENGINES
// выше) как управляемый дочерний процесс через псевдотерминал вместо
// отдельного окна ОС. cwd выставляем явно в AUTOMAXKG_DIR — раньше рабочую
// директорию выставляла сама ОС по местоположению файла (как при двойном
// клике), здесь мы её задаём напрямую тем же результатом. Запускаем
// напрямую из AUTOMAXKG_DIR, без какого-либо промежуточного шага (файлы не
// шифруются — защита только на уровне ФС, см. комментарий у
...
// падает с "не является внутренней или внешней командой". Логика теперь
// инкапсулирована в ENGINES.automaxkg.spawn() — здесь она не дублируется.
//
...
ipcMain.handle('automaxkg-terminal-start', async (event, { cols, rows, engineId }) => {
  if (activePty) {
    return { ok: false, error: 'Программа обработки уже запущена' };
  }
  const engine = getEngine(engineId);
  if (!engine.isPresent()) {
    return { ok: false, error: `Файлы ${engine.label} не найдены на этом компьютере` };
  }

  try {
    activePty = engine.spawn(cols, rows);
  } catch (err) {
    activePty = null;
    // engineId в ошибке — чтобы в логе сразу было видно, какой именно
    // движок упал на запуске, если моделей/движков станет больше одного.
    log.error(`Не удалось запустить движок "${engineId || 'automaxkg'}"`, err);
